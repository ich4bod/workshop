...
const render = (data) => {
  ...
  const reading = document.createElement('section'); reading.className = 'reading'; reading.innerHTML = `<h2>What the fixture makes visible</h2><p><strong>Agreement:</strong> ${escape(data.agreement)}</p><p><strong>Conflict:</strong> ${escape(data.conflict)}</p>`; worksheet.append(reading);
  data.options.forEach(addOption);
};
const escape = (text = '') => String(text).replace(/[&<>"']/g, (char) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[char]);
const snapshot = () => ({ title: fixture.title, question: fixture.question, agreement: fixture.agreement, conflict: fixture.conflict, options: [...worksheet.querySelectorAll('.option')].map(optionValue) });
const report = (data) => `<!doctype html><meta charset="utf-8"><title>Uncertainty worksheet</title><style>body{max-width:800px;margin:3rem auto;padding:0 1rem;font:16px system-ui;line-height:1.5}article,section{border:1px solid #aaa;padding:1rem;margin:1rem 0}h1,h2{line-height:1.15}</style><h1>${escape(data.title)}</h1><p>${escape(data.question)}</p><section><strong>Agreement:</strong> ${escape(data.agreement)}<br><strong>Conflict:</strong> ${escape(data.conflict)}</section>${data.options.map((option) => `<article><h2>${escape(option.name || 'Unlabelled option')}</h2><p><strong>Confidence:</strong> ${escape(option.confidence)}</p><p><strong>Assumptions:</strong><br>${option.assumptions.map(escape).join('<br>') || 'Not recorded'}</p><p><strong>Disconfirming evidence:</strong><br>${option.disconfirmingEvidence.map(escape).join('<br>') || 'Not recorded'}</p><p><strong>Next check:</strong> ${escape(option.nextCheck) || 'Not recorded'}</p></article>`).join('')}<p><em>This worksheet records uncertainty; it does not rank options.</em></p>`;
const download = () => { const link = Object.assign(document.createElement('a'), { href: URL.createObjectURL(new Blob([report(snapshot())], { type: 'text/html' })), download: 'uncertainty-worksheet.html' }); link.click(); setTimeout(() => URL.revokeObjectURL(link.href), 0); };
