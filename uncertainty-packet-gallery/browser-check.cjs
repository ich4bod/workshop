...
server.listen(8774, '127.0.0.1', async () => {
  ...
  try {
    ...
    console.log('browser check passed: three scenarios rendered; every packet exposed claims, sources, confidence, conflict, next checks, and gaps without a verdict, and all scenarios remained usable in a network-disabled file:// browser');
