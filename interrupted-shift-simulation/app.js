...
function choose(a) {
  ...
  state.picked=true; state.grid+=a.grid; state.trust+=a.trust; state.actions.push({name:a.name, kept:a.memory}); if(a.memory) state.notes.push(a.memory); $('grid').textContent=state.grid+'/7'; $('trust').textContent=state.trust+'/7'; $('notes-count').textContent=state.notes.length;
